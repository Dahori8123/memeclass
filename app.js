...
class DVDBouncer {
    ...
    addItems(itemsList) {
        ...
        itemsList.forEach(item => {
            ...
            if (item.isPfp) {
                ...
                img.className = 'w-32 h-32 md:w-48 md:h-48 rounded-2xl border-4 border-white shadow-lg object-cover';
            }
            el.appendChild(img);
    ...
    animate() {
        this.items.forEach(item => {
            if (item.w === 0) {
                item.w = item.el.offsetWidth || 192; // md:w-48 is 192px
                item.h = item.el.offsetHeight || 192;
            }
